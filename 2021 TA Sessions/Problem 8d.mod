#One market can be supplied by multiple factories
...
var Y {FACTORY, MARKET} binary;	# 1 - if there is a shipment from factory f to market m, 0 - otherwise
var W {MARKET} binary;			# 1 - if a market m has multiple suppliers

minimize Costs:
	  sum{f in FACTORY, m in MARKET} c[f,m]*X[f,m] + sum{f in FACTORY} fc[f]*Z[f];
...
s.t. Constraint3_DefineY {f in FACTORY, m in MARKET}:
	X[f,m] <= cap[f]*Y[f,m];

s.t. Constraint4_SingleSourcing {m in MARKET}:
	sum{f in FACTORY} Y[f,m] <= 1;
	
s.t. Constraint5_OneMarketMultipleSuppliers:
	sum{m in MARKET} W[m] <= 1;
	
s.t. Constraint6_DefineW {m in MARKET}:
	W[m] >= (sum{f in FACTORY} Y[f,m] - 1) / M;
